...
interface ProfilePageProps {
    ...
    setSettings: (value: GameSettings | ((val: GameSettings) => GameSettings)) => void;
    stories: Story[];
    playthroughs: Playthrough[];
    onEdit: (story: Story) => void;
    onDeleteStory: (storyId: string) => void;
    onPlay: (story: Story) => void;
    onDeletePlaythrough: (storyId: string) => void;
}
...
const ProfilePage: React.FC<ProfilePageProps> = ({ settings, setSettings, stories, playthroughs, onEdit, onDeleteStory, onPlay, onDeletePlaythrough }) => {
    ...
    const myStories = user ? stories.filter(s => s.creatorId === user.id) : [];
    ...
    const handleConfirm = () => {
        ...
        if (confirmAction.type === 'deleteStory') {
            onDeleteStory(confirmAction.id);
        } else if (confirmAction.type === 'deletePlaythrough') {
            onDeletePlaythrough(confirmAction.id);
        }
                        ...
                        recentlyPlayedStories.map(story => (
                            <div key={story.id} className="relative group">
                               ...
                               <div className="absolute inset-0 bg-black/60 rounded-lg flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => onPlay(story)} className="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-md hover:bg-indigo-700">{t.continue}</button>
